...
export function getPostsData() {
    ...
    const allPostsData = fileNames.map((fileName) => {
        // 拡張子は除去しファイル名(id)を取得
        const id_life = fileName.replace(/\.md$/, "");
...
// idに基づいてブログ投稿データを返す
export async function getPostData(id_life) {
    // マークダウンファイルを文字列として読み取る
    const fullPath = path.join(postsDirectory, `${id_life}.md`);
